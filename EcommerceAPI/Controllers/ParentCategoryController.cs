    ...
    public class ParentCategoryApiController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<ParentCategory>>> GetParentCategories()
        {
            var parentCategories = await _dbContext.ParentCategories
                ...
                .Include(pc => pc.Categories.Where(pc => pc.IsDeleted == false && pc.Products.Any()))
                .ToListAsync();
        ...
        {
            var parentCategories = await _dbContext.ParentCategories
                .Where(pc => pc.IsDeleted == false && pc.Gender == (int)gender &&
                             pc.Categories.Any(c => c.IsDeleted == false && 
                                                    c.Products.Any(p => p.IsDeleted == false &&
                                                        p.ProductVariations.Any(pv => pv.IsDeleted == false))))
                .Include(pc => pc.Categories.Where(c => c.IsDeleted == false &&
                                                        c.Products.Any(p => p.IsDeleted == false &&
                                                                            p.ProductVariations.Any(pv => pv.IsDeleted == false))))
                .ToListAsync();
